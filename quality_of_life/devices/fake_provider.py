...
class FakePhoneProvider:
    ...
    def add_device(
        ...
        self.device_capabilities[state.device_id] = capabilities or frozenset(
    ...
    def _require_capability(
        self, device_id: str, capability: DeviceCapability
    ) -> DeviceResult | None:
    ...
    def view_screen(self, device_id: str) -> DeviceResult:
        for failure in (
            ...
            self._require_capability(device_id, DeviceCapability.SCREEN_VIEW),
        ):
    ...
    def send_input(self, device_id: str, event: DeviceInputEvent) -> DeviceResult:
        for failure in (
            self._require_connected(device_id),
            self._require_capability(device_id, DeviceCapability.INPUT_CONTROL),
        ):
            if failure is not None:
                return failure
    ...
    def read_notifications(self, device_id: str) -> DeviceResult:
        for failure in (
            self._require_connected(device_id),
            self._require_capability(device_id, DeviceCapability.NOTIFICATION_READ),
        ):
            if failure is not None:
                return failure
    ...
    def transfer_file(
        ...
        for failure in (
            self._require_connected(device_id),
            self._require_capability(device_id, DeviceCapability.FILES),
        ):
            if failure is not None:
                return failure
    ...
    def open_app(self, device_id: str, app_id: str) -> DeviceResult:
        for failure in (
            self._require_connected(device_id),
            self._require_capability(device_id, DeviceCapability.APP_CONTROL),
        ):
            if failure is not None:
                return failure
        if not app_id.strip():
